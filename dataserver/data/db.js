...
function UserAchievementGetByUserIDAndArcID(userID, arcID) {
    return knex("userachievement").select("*").where({userID: userID, arcID: arcID});
};
...
function UserAchievementUpdateLevel(userID, arcID, info) {
    return knex("userachievement").where({userID: userID, arcID: arcID}).update(info);
};
...
function ItemGetByUserId(userID, isOwned)
{
    if (isOwned === "true")
    {
        return knex("item").select("*").whereIn("itemID", function(){
            this.select("itemID").from("inventory").where("userID", userID)
        }).orderBy("itemID")
    }
    else
    {
        return knex("item").select("*").whereNotIn("itemID", function(){
            this.select("itemID").from("inventory").where("userID", userID)
        }).orderBy("itemID")
    }
};
...
function DungeonStatGetProgress(userID, dungeonID, difficulty) {
    return knex("dungeonstat").select("*").where({userID: userID, dungeonID: dungeonID, difficulty: difficulty});
};

function DungeonStatGetSumTotal(userID, dungeonID) {
    return knex("dungeonstat").sum("total as sum").where({userID: userID, dungeonID: dungeonID});
};

function DungeonStatUpdate(userID, dungeonID, difficulty, isComplete) {
    if (isComplete === "true")
    {
        return knex("dungeonstat").where({userID: userID, dungeonID: dungeonID, difficulty: difficulty}).increment({daily: 1, weekly: 1, total: 1});
    } 
    else 
    {
        return knex("dungeonstat").where({userID: userID, dungeonID: dungeonID, difficulty: difficulty}).increment({fail: 1});
    }
};
